...
def cosine_similarity(query_vec, doc_vecs):
    ...
    This is what ChromaDB was doing internally — we do it manually
    with numpy, which works everywhere!
    """
...
def retrieve_relevant_chunks(query, n_results=4):
    ...
    chunks  = [st.session_state.chunks_store[i]  for i in top_idx]
    sources = [st.session_state.chunk_sources[i] for i in top_idx]
...
if process_btn and uploaded_files and api_key:
    with st.spinner("Processing documents..."):
        ...
        st.sidebar.success(f"✅ Done! {len(all_chunks)} chunks indexed")
...
st.markdown('<p class="title-text">🤖 RAG Document Chatbot</p>',
            unsafe_allow_html=True)
st.markdown('<p class="subtitle-text">Ask questions about your PDF or TXT documents · '
            'Powered by Groq LLaMA 3.1 + FAISS + Sentence Transformers</p>',
            unsafe_allow_html=True)
...
with c1:
    st.markdown('<div class="metric-card"><div class="metric-label">LLM</div>'
                ...
                unsafe_allow_html=True)
with c2:
    st.markdown('<div class="metric-card"><div class="metric-label">Search</div>'
                '<div class="metric-value">FAISS</div></div>',
                unsafe_allow_html=True)
with c3:
...
with info_col:
    ...
    <div style='background:#1e2330; border-radius:12px; padding:1.2rem;'>
        ...
        <div style='color:#94a3b8; font-size:0.85rem; margin-bottom:1rem'>
            ...
            converted to 384-dim vectors using
